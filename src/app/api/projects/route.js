...
export const POST = withAuth(async function (req) {
  try {
    await dbConnect();
    const { name, description, category, clientName, clientEmail, clientPhone, status, createdBy, members, startDate, endDate, documents, budget, needSiteSurvey, currency } = await req.json();
